...
clean_egfr_medcodes <- clean_creatinine %>%
...
qdhf <- example_dataset %>%
  ...
  analysis$cached("qdhf", indexes=c("patid", "dstartdate", "drugclass"))
...
qdhf_and_qrisk2 <- qdhf %>%
  ...
  analysis$cached("qdhf_and_qrisk2", indexes=c("patid", "dstartdate", "drugclass"))
