...
calculate_ckdpc_egfr60_risk = function(dataframe, age, sex, black_eth, egfr, cvd, hba1c, insulin, oha, ever_smoker, hypertension, bmi, acr, remote) {
  ...
  new_dataframe <- new_dataframe %>%
    select(id_col, ckdpc_egfr60_risk_total_score, ckdpc_egfr60_risk_total_score2, ckdpc_egfr60_risk_total_lin_predictor, ckdpc_egfr60_risk_confirmed_score, ckdpc_egfr60_risk_confirmed_score2, ckdpc_egfr60_risk_confirmed_lin_predictor)
...
calculate_ckdpc_40egfr_risk = function(dataframe, age, sex, egfr, acr, sbp, bp_meds, hf, chd, af, current_smoker, ex_smoker, bmi, hba1c, oha, insulin, remote) {
  ...
  new_dataframe <- new_dataframe %>%
    ...
    mutate(male_sex=ifelse(!!sex_col=="male", 1L, 0L),
           ...
           ckdpc_40egfr_risk_score2=100 * (exp(ckdpc_40egfr_risk_lin_predictor2 + intercept)/(1 + exp(ckdpc_40egfr_risk_lin_predictor2 + intercept))))
